...
ix.lang.AddTable("english", {
	optItemESP = "ESP на предметы",
	optdItemESP = "Показывает имена и расположение каждого элемента на сервере.",
})
		...
		if (client:IsAdmin() and client:GetMoveType() == MOVETYPE_NOCLIP and not client:InVehicle() and ix.option.Get("observerESP", true)) then
					...
					for k2, v2 in pairs(EntitiesESPList) do 
						if string.find(k2, v:GetClass()) then
							...
							if entities == "ix_item" then
								...
								ix.util.DrawText("[Предмет]" ..itemName.name, x2, y2 - size2, entcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha2)
							elseif entities == "ix_vendor" then
								local NPCName = v:GetDisplayName()
								ix.util.DrawText("[Вендор]" ..NPCName, x2, y2 - size2, entcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha2)
							...
							elseif entities == "ix_container" then
								local EntName = v:GetDisplayName()
								ix.util.DrawText("[Контейнер]" ..EntName, x2, y2 - size2, entcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha2)
							elseif entities == "ix_money" then
								local Amount = v:GetAmount()
								ix.util.DrawText("[Деньги]" ..Amount, x2, y2 - size2, entcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha2)
							...
							elseif entities == "ix_forcefield" then
								ix.util.DrawText("[Постройка]", x2, y2 - size2, entcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha2)
			...
			for _, v in ipairs(player.GetAll()) do
				...
				if not v.status then
					v.status = "user"
				elseif v:IsUserGroup("founder") then
					v.status = "Основатель"
				...
				local text3 = "ХП: " .. v:Health() .. " Броня: " .. v:Armor()
